#!/usr/bin/env python3
"""Run the standard ANN-NM-MPOD OpInf comparison points."""
...
from run_ann_manifold_opinf import DEFAULT_MODEL_PATH, main as run_ann_manifold_opinf
...
def _plot_suite_errors(rows, out_path):
    ...
    bars = ax.bar(x, errors, color="#b07aa1", edgecolor="#70496a", linewidth=0.8)
    ...
    ax.set_title("ANN-NM-MPOD OpInf suite rollout errors")
...
def main(
    ...
    print("\n====================================================")
    print("        1D ANN-NM-MPOD OPINF TEST SUITE")
    print("====================================================")

    for i, (mu1, mu2) in enumerate(test_points, start=1):
        print("\n----------------------------------------------------")
        print(f"[ANN-OpInf-SUITE] Case {i}/{len(test_points)}: mu1={mu1:.3f}, mu2={mu2:.4f}")
        print("----------------------------------------------------")
        t0 = time.time()
        _, rel_err = run_ann_manifold_opinf(
            mu1=mu1,
            mu2=mu2,
    ...
    csv_path = os.path.join(results_dir, "ann_opinf_suite_summary.csv")
    txt_path = os.path.join(results_dir, "ann_opinf_suite_summary.txt")
    plot_path = os.path.join(results_dir, "ann_opinf_suite_errors.png")
    ...
    with open(txt_path, "w", encoding="utf-8") as file:
        ...
        file.write("script: OpInf/run_ann_manifold_opinf_suite.py\n\n")
    ...
    print(f"\n[ANN-OpInf-SUITE] Summary CSV saved: {csv_path}")
    print(f"[ANN-OpInf-SUITE] Error plot saved: {plot_path}")
    print(f"[ANN-OpInf-SUITE] Summary TXT saved: {txt_path}")
    return rows


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the standard ANN-NM-MPOD OpInf comparison points.")
