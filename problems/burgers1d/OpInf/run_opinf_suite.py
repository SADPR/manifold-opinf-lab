#!/usr/bin/env python3
"""Run the standard linear discrete-time OpInf ROM comparison points."""
...
from run_opinf import DEFAULT_MODEL_PATH, main as run_opinf
...
def _plot_suite_errors(rows, out_path):
    ...
    ax.set_title("Linear OpInf suite rollout errors")
...
def main(
    ...
    print("\n====================================================")
    print("       1D LINEAR DISCRETE OPINF TEST SUITE")
    print("====================================================")

    for i, (mu1, mu2) in enumerate(test_points, start=1):
        print("\n----------------------------------------------------")
        print(f"[OpInf-SUITE] Case {i}/{len(test_points)}: mu1={mu1:.3f}, mu2={mu2:.4f}")
        print("----------------------------------------------------")
        t0 = time.time()
        _, rel_err = run_opinf(
            mu1=mu1,
    ...
    csv_path = os.path.join(results_dir, "opinf_linear_discrete_suite_summary.csv")
    txt_path = os.path.join(results_dir, "opinf_linear_discrete_suite_summary.txt")
    plot_path = os.path.join(results_dir, "opinf_linear_discrete_suite_errors.png")
    ...
    with open(txt_path, "w", encoding="utf-8") as file:
        ...
        file.write("script: OpInf/run_opinf_suite.py\n\n")
    ...
    print(f"\n[OpInf-SUITE] Summary CSV saved: {csv_path}")
    print(f"[OpInf-SUITE] Error plot saved: {plot_path}")
    print(f"[OpInf-SUITE] Summary TXT saved: {txt_path}")
    return rows


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run the standard linear OpInf comparison points.")
