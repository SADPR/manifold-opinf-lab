#!/usr/bin/env python3
"""Run one quadratic discrete-time OpInf ROM case."""
...
from opinf_utils import (
    ...
    rollout_quadratic_discrete,
)
from run_opinf import _plot_error_history, _relative_error_history, _safe_tag


DEFAULT_MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "quadratic_discrete_n10.npz")
...
def main(
    ...
    if model["model_family"] != QUADRATIC_MODEL_FAMILY:
        raise ValueError(f"Expected {QUADRATIC_MODEL_FAMILY}, got {model['model_family']}.")
    ...
    print(f"[OpInf-Quad] HDM snapshot file: {hdm_path}")
    if hdm_was_cached:
        print("[OpInf-Quad] HDM snapshots found. Loading before OpInf rollout.")
    else:
        print("[OpInf-Quad] HDM snapshots not found. Computing before OpInf rollout.")
    ...
    print(f"[OpInf-Quad] Loaded model: {model_path}")
    print(f"[OpInf-Quad] POD basis: {basis_path}")
    print(f"[OpInf-Quad] Basis size used: {n_keep}")
    print(f"[OpInf-Quad] Training one-step error: {model['train_relative_one_step_error']:.3e}")
    quadratic_param_mode = model.get("quadratic_param_mode", "parametric_H") or "parametric_H"
    include_param_quad = quadratic_param_mode == "parametric_H"
    include_param_linear = quadratic_param_mode in ("constant_H", "parametric_H")
    print(f"[OpInf-Quad] Quadratic parameter mode: {quadratic_param_mode}")

    t0 = time.time()
    q_rom, stable_steps, unstable_reason = rollout_quadratic_discrete(
        q0,
        mu,
        num_steps,
    ...
    tag = _safe_tag(mu)
    rom_path = os.path.join(results_dir, f"opinf_quadratic_discrete_{tag}.npy")
    q_path = os.path.join(results_dir, f"opinf_quadratic_discrete_q_{tag}.npy")
    err_path = os.path.join(results_dir, f"opinf_quadratic_discrete_error_history_{tag}.npy")
    np.save(rom_path, rom_snaps)
    np.save(q_path, q_rom)

    fig_path = os.path.join(results_dir, f"opinf_quadratic_discrete_{tag}.png")
    plot_snaps(
        GRID_X,
        hdm_snaps,
        rom_snaps=rom_snaps,
        ...
        out_path=fig_path,
        title=f"HDM/Quadratic OpInf 1D Burgers, mu=({mu[0]:.2f}, {mu[1]:.3f})",
    )

    rel_error_history = _relative_error_history(hdm_snaps, rom_snaps)
    np.save(err_path, rel_error_history)
    error_fig_path = os.path.join(results_dir, f"opinf_quadratic_discrete_error_{tag}.png")
    times = float(dt) * np.arange(int(num_steps) + 1, dtype=np.float64)
    _plot_error_history(
        times,
        rel_error_history,
        error_fig_path,
        title=f"Quadratic OpInf error history, mu=({mu[0]:.2f}, {mu[1]:.3f})",
    )

    report_path = os.path.join(results_dir, f"opinf_quadratic_discrete_summary_{tag}.txt")
    with open(report_path, "w", encoding="utf-8") as file:
        file.write("[run]\n")
        file.write(f"timestamp: {datetime.now().isoformat(timespec='seconds')}\n")
        file.write("script: OpInf/run_quadratic_opinf.py\n\n")
        file.write("[configuration]\n")
        file.write(f"model_family: {QUADRATIC_MODEL_FAMILY}\n")
        ...
        file.write(f"num_features: {model.get('num_features', model['operator'].shape[1])}\n")
        file.write(f"ridge: {model['ridge']:.8e}\n")
        file.write(f"feature_mode: {model['feature_mode']}\n")
        file.write(f"quadratic_param_mode: {quadratic_param_mode}\n")
    ...
    print(f"[OpInf-Quad] Relative error over finite snapshots: {rel_err:.3e}")
    if np.isfinite(final_rel_err):
        print(f"[OpInf-Quad] Final snapshot relative error: {final_rel_err:.3e}")
    else:
        print(f"[OpInf-Quad] Final snapshot relative error: NaN ({unstable_reason})")
    print(f"[OpInf-Quad] Summary saved: {report_path}")
    print(f"[OpInf-Quad] Snapshot overlay saved: {fig_path}")
    print(f"[OpInf-Quad] Error plot saved: {error_fig_path}")
    return rom_snaps, rel_err


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run one quadratic discrete-time OpInf ROM test point.")
