#!/usr/bin/env python3
"""Run one full-center RBF-manifold continuous-time OpInf ROM case."""
...
from rbf_manifold_opinf_utils import (
    load_rbf_manifold_model,
    rbf_manifold_decode,
    rollout_rbf_continuous_rk4,
)
from run_opinf import _plot_error_history, _relative_error_history, _safe_tag


DEFAULT_MODEL_PATH = os.path.join(SCRIPT_DIR, "models", "rbf_manifold_linear_plus_rbf_r20_q40.npz")
...
def main(
    ...
    results_dir=os.path.join(PROJECT_ROOT, "Results", "OpInf-RBF-Manifold"),
    dt=DT,
    num_steps=NUM_STEPS,
    max_norm=1e12,
):
    ...
    model = load_rbf_manifold_model(model_path)
    ...
    print(f"[RBF-OpInf] HDM snapshot file: {hdm_path}")
    if hdm_was_cached:
        print("[RBF-OpInf] HDM snapshots found. Loading before RBF-manifold OpInf rollout.")
    else:
        print("[RBF-OpInf] HDM snapshots not found. Computing before RBF-manifold OpInf rollout.")
    ...
    print(f"[RBF-OpInf] Loaded model: {model_path}")
    print(f"[RBF-OpInf] POD basis: {basis_path}")
    print(f"[RBF-OpInf] Primary modes r={n_primary}, secondary modes q={n_secondary}")
    print(f"[RBF-OpInf] RBF centers: {model['num_rbf_centers']}")
    print(
        "[RBF-OpInf] RBF: "
        f"kernel={model['rbf_kernel_name']}, eps={model['rbf_epsilon']:.3e}, "
        f"ridge={model['rbf_ridge']:.3e}, cv_error={model['rbf_cv_relative_error']:.3e}"
    )
    print(f"[RBF-OpInf] Manifold training error: {model['relative_manifold_training_error']:.3e}")
    print(f"[RBF-OpInf] Derivative training error: {model['relative_derivative_training_error']:.3e}")

    t0 = time.time()
    q_rom, stable_steps, unstable_reason = rollout_rbf_continuous_rk4(
        q0,
        mu,
        dt,
        num_steps,
    ...
    tag = _safe_tag(mu)
    rom_path = os.path.join(results_dir, f"rbf_opinf_snaps_{tag}.npy")
    q_path = os.path.join(results_dir, f"rbf_opinf_q_{tag}.npy")
    err_path = os.path.join(results_dir, f"rbf_opinf_error_history_{tag}.npy")
    np.save(rom_path, rom_snaps)
    np.save(q_path, q_rom)

    fig_path = os.path.join(results_dir, f"rbf_opinf_{tag}.png")
    plot_snaps(
        GRID_X,
        hdm_snaps,
        rom_snaps=rom_snaps,
        ...
        out_path=fig_path,
        title=f"HDM/RBF-Manifold OpInf 1D Burgers, mu=({mu[0]:.2f}, {mu[1]:.3f})",
    )

    rel_error_history = _relative_error_history(hdm_snaps, rom_snaps)
    np.save(err_path, rel_error_history)
    error_fig_path = os.path.join(results_dir, f"rbf_opinf_error_{tag}.png")
    times = float(dt) * np.arange(int(num_steps) + 1, dtype=np.float64)
    _plot_error_history(
        times,
        rel_error_history,
        error_fig_path,
        title=f"RBF-Manifold OpInf error history, mu=({mu[0]:.2f}, {mu[1]:.3f})",
    )

    report_path = os.path.join(results_dir, f"rbf_opinf_summary_{tag}.txt")
    with open(report_path, "w", encoding="utf-8") as file:
        file.write("[run]\n")
        file.write(f"timestamp: {datetime.now().isoformat(timespec='seconds')}\n")
        file.write("script: OpInf/run_rbf_manifold_opinf.py\n\n")
        file.write("[configuration]\n")
        file.write("model_family: rbf_manifold_continuous\n")
        ...
        file.write(f"num_rbf_centers: {model['num_rbf_centers']}\n")
        file.write(f"rbf_include_mu: {bool(model['rbf_include_mu'])}\n")
        file.write(f"rbf_kernel_name: {model['rbf_kernel_name']}\n")
        file.write(f"rbf_epsilon: {model['rbf_epsilon']:.8e}\n")
        file.write(f"rbf_ridge: {model['rbf_ridge']:.8e}\n")
        file.write(f"rbf_cv_relative_error: {model['rbf_cv_relative_error']:.8e}\n")
        ...
        file.write(f"include_rbf_dynamics: {bool(model['include_rbf_dynamics'])}\n")
        file.write(f"include_param_rbf_dynamics: {bool(model['include_param_rbf_dynamics'])}\n")
    ...
    print(f"[RBF-OpInf] Relative error over finite snapshots: {rel_err:.3e}")
    if np.isfinite(final_rel_err):
        print(f"[RBF-OpInf] Final snapshot relative error: {final_rel_err:.3e}")
    else:
        print(f"[RBF-OpInf] Final snapshot relative error: NaN ({unstable_reason})")
    print(f"[RBF-OpInf] Summary saved: {report_path}")
    print(f"[RBF-OpInf] Snapshot overlay saved: {fig_path}")
    print(f"[RBF-OpInf] Error plot saved: {error_fig_path}")
    return rom_snaps, rel_err


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run one RBF-manifold OpInf test point.")
    ...
    parser.add_argument("--results-dir", default=os.path.join(PROJECT_ROOT, "Results", "OpInf-RBF-Manifold"))
