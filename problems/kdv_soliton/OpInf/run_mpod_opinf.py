#!/usr/bin/env python3
"""Run POD-based polynomial-manifold OpInf for the KdV soliton."""
...
def main(
    ...
    snapshot_file=None,
    results_dir=os.path.join(PROJECT_ROOT, "Results", "OpInf", "MPOD"),
    max_norm=1e6,
):
    ...
    q_rom, unstable, unstable_index = rollout_rk4(q_fom[:, 0], times, coeffs, exponents, max_norm=max_norm)
    rom = reconstruct_mpod_snapshots(q_rom, basis, basis_bar, xi, u_ref, degree)
    ...
    spacetime_plot = os.path.join(results_dir, f"mpod_opinf_{tag}_spacetime.png")
    snapshot_plot = os.path.join(results_dir, f"mpod_opinf_{tag}_snapshots.png")
    error_plot = os.path.join(results_dir, f"mpod_opinf_{tag}_error_history.png")
    q_path = os.path.join(results_dir, f"mpod_opinf_{tag}_q_rollout.npz")
    summary_path = os.path.join(results_dir, f"mpod_opinf_{tag}_summary.txt")
    ...
    write_txt_report(
        summary_path,
        [
            ("run", [("timestamp", datetime.now().isoformat(timespec="seconds")), ("script", "OpInf/run_mpod_opinf.py")]),
            (
                "model",
                [
    ...
    print(f"[KDV-MPOD] train error={train_error:.6e}")
    print(f"[KDV-MPOD] prediction error={prediction_error:.6e}")
    print(f"[KDV-MPOD] full error={full_error:.6e}")
    print(f"[KDV-MPOD] summary saved: {summary_path}")
    return rom, full_error


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run MPOD-OpInf for KdV.")
    ...
    parser.add_argument("--results-dir", default=os.path.join(PROJECT_ROOT, "Results", "OpInf", "MPOD"))
