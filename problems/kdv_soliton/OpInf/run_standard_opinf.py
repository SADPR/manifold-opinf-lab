#!/usr/bin/env python3
"""Run standard linear-subspace quadratic OpInf for the KdV soliton."""
...
def main(
    ...
    snapshot_file=None,
    results_dir=os.path.join(PROJECT_ROOT, "Results", "OpInf", "Standard"),
    max_norm=1e6,
):
    ...
    q_rom, unstable, unstable_index = rollout_rk4(q_fom[:, 0], times, coeffs, max_norm=max_norm)
    rom = reconstruct_snapshots(q_rom, basis, u_ref)
    ...
    tag = f"r{num_modes}"
    spacetime_plot = os.path.join(results_dir, f"standard_opinf_{tag}_spacetime.png")
    snapshot_plot = os.path.join(results_dir, f"standard_opinf_{tag}_snapshots.png")
    error_plot = os.path.join(results_dir, f"standard_opinf_{tag}_error_history.png")
    q_path = os.path.join(results_dir, f"standard_opinf_{tag}_q_rollout.npz")
    summary_path = os.path.join(results_dir, f"standard_opinf_{tag}_summary.txt")
    ...
    plot_spacetime_comparison(
        ...
        train_final_time,
        spacetime_plot,
        title=f"KdV standard OpInf, r={num_modes}",
    )
    plot_snapshot_comparison(x, times, snapshots, rom, snapshot_plot, snapshot_times=(train_final_time, times[-1]))
    plot_error_history(times, error_history, train_final_time, error_plot)
    np.savez(
        q_path,
        times=times,
        q_fom=q_fom,
    ...
    write_txt_report(
        summary_path,
        [
            ("run", [("timestamp", datetime.now().isoformat(timespec="seconds")), ("script", "OpInf/run_standard_opinf.py")]),
            (
                "model",
                [
                    ...
                    ("num_modes", num_modes),
                    ("regularizer_c", model["ridge_c"]),
                    ("regularizer_a", model["ridge_a"]),
                    ("regularizer_h", model["ridge_h"]),
                    ("regularizer_convention", model.get("regularizer_convention", "opinf_direct_weight")),
                    ("energy_captured_training_pod", model["energy_captured"]),
    ...
    print("\n====================================================")
    print("            KDV STANDARD OPINF ROLLOUT")
    print("====================================================")
    print(f"[KDV-OpInf] model={model_path}")
    print(
        f"[KDV-OpInf] r={num_modes}, "
        f"regularizer=(c={float(model['ridge_c']):.3e}, A={float(model['ridge_a']):.3e}, "
        f"H={float(model['ridge_h']):.3e})"
    )
    print(f"[KDV-OpInf] train error={train_error:.6e}")
    print(f"[KDV-OpInf] prediction error={prediction_error:.6e}")
    print(f"[KDV-OpInf] full error={full_error:.6e}")
    print(f"[KDV-OpInf] summary saved: {summary_path}")
    return rom, full_error


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Run standard quadratic OpInf for KdV.")
    ...
    parser.add_argument("--results-dir", default=os.path.join(PROJECT_ROOT, "Results", "OpInf", "Standard"))
