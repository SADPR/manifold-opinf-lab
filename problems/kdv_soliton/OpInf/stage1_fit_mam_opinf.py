#!/usr/bin/env python3
"""Fit alternating-minimization manifold OpInf for the KdV soliton."""
...
def main(
    ...
    print("\n====================================================")
    print("             KDV MAM-OPINF TRAINING")
    print("====================================================")
    print(f"[KDV-MAM] snapshot_file={snapshot_file}")
    print(f"[KDV-MAM] r={r}, q={q_secondary}, total_modes={total}, p={p}, gamma={float(gamma):.3e}")
    print(
        f"[KDV-MAM] AM tol={float(am_tol):.1e}, max_iter={int(am_max_iter)}, "
        f"ls_ftol={float(ls_ftol):.1e}, coordinate_initialization={coordinate_initialization}"
    )
    print(f"[KDV-MAM] train snapshots={train_snapshots.shape[1]}, train window=[{train_times[0]:.4f}, {train_times[-1]:.4f}]")

    manifold = compute_mam_manifold(
        train_snapshots,
        num_modes=r,
        total_modes=total,
        degree=p,
        gamma=float(gamma),
        tol=float(am_tol),
        max_iter=int(am_max_iter),
        ls_ftol=float(ls_ftol),
        ls_max_nfev=ls_max_nfev,
        coordinate_initialization=coordinate_initialization,
        progress=True,
    )
    ...
    print(f"[KDV-MAM] initial MPOD reconstruction error={manifold['initial_mpod_reconstruction_error']:.3e}")
    print(f"[KDV-MAM] initial MPOD energy metric={manifold['initial_mpod_energy_captured']:.3e}")
    print(f"[KDV-MAM] MAM reconstruction error={manifold['relative_reconstruction_error']:.3e}")
    print(f"[KDV-MAM] MAM energy metric={manifold['energy_captured']:.3e}")
    print(f"[KDV-MAM] AM converged={manifold['converged']}, iterations={manifold['iterations']}")
    print(f"[KDV-MAM] OpInf features={num_features} (higher={exponents.shape[0]})")
    ...
    for regularizer_ca in regularizer_ca_candidates:
        for regularizer_h in regularizer_h_candidates:
            for regularizer_p in regularizer_p_candidates:
                ...
                train_rom = reconstruct_mam_snapshots(q_rollout, basis, basis_bar, xi, u_ref, p)
    ...
    if best is None or not np.isfinite(best["row"]["training_rollout_error"]):
        raise RuntimeError("All MAM-OpInf regularizer candidates were unstable.")

    selected = best["row"]
    selected_fit = best["fit"]
    history_arrays = history_to_arrays(manifold["history"])
    save_model(
        model_path,
        ...
        am_tol=np.asarray(float(am_tol), dtype=np.float64),
        am_iterations=np.asarray(int(manifold["iterations"]), dtype=np.int64),
        am_converged=np.asarray(int(bool(manifold["converged"])), dtype=np.int64),
        ls_ftol=np.asarray(float(ls_ftol), dtype=np.float64),
        coordinate_initialization=np.asarray(str(coordinate_initialization)),
        ...
        initial_mpod_reconstruction_error=np.asarray(float(manifold["initial_mpod_reconstruction_error"]), dtype=np.float64),
        initial_mpod_energy_captured=np.asarray(float(manifold["initial_mpod_energy_captured"]), dtype=np.float64),
    ...
    tag = f"mam_opinf_r{r}_p{p}_q{q_secondary}"
    ...
    basis_plot = os.path.join(results_dir, f"{tag}_basis_comparison.png")
    plot_basis_comparison(x, manifold["pod_basis"][:, :r], basis, basis_plot, num_vectors=min(4, r))

    history_csv = os.path.join(results_dir, f"{tag}_am_history.csv")
    with open(history_csv, "w", encoding="utf-8") as file:
        file.write("iteration,energy,diff,relative_reconstruction_error,nfev_total,nfev_max\n")
        for row in manifold["history"]:
            file.write(
                f"{row['iteration']},{row['energy']:.16e},{row['diff']:.16e},"
                f"{row['relative_reconstruction_error']:.16e},{row['nfev_total']},{row['nfev_max']}\n"
            )
    ...
    write_txt_report(
        summary_path,
        [
            ("run", [("timestamp", datetime.now().isoformat(timespec="seconds")), ("script", "OpInf/stage1_fit_mam_opinf.py")]),
            (
                "data",
                [
            ...
            (
                "manifold",
                [
                    ("model_family", "alternating-minimization polynomial-manifold OpInf"),
                    ...
                    ("gamma", float(gamma)),
                    ("am_tol", float(am_tol)),
                    ("am_iterations", int(manifold["iterations"])),
                    ("am_converged", bool(manifold["converged"])),
                    ("ls_ftol", float(ls_ftol)),
                    ("coordinate_initialization", str(coordinate_initialization)),
                    ("initial_mpod_reconstruction_error", manifold["initial_mpod_reconstruction_error"]),
                    ("initial_mpod_energy_captured", manifold["initial_mpod_energy_captured"]),
            ...
            (
                ...
                [
                    ...
                    ("regularizer_grid_csv", csv_path),
                    ("am_history_csv", history_csv),
                    ("singular_values_png", singular_plot),
                    ("basis_comparison_png", basis_plot),
    ...
    print(
        "[KDV-MAM] selected regularizer weights="
        f"(ca={selected['regularizer_ca']:.3e}, h={selected['regularizer_h']:.3e}, "
        f"p={selected['regularizer_p']:.3e})"
    )
    print(f"[KDV-MAM] model saved: {model_path}")
    print(f"[KDV-MAM] summary saved: {summary_path}")
    return model_path


if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Fit MAM-OpInf for KdV.")
    ...
    parser.add_argument("--am-tol", type=float, default=1e-3)
    parser.add_argument("--am-max-iter", type=int, default=100)
    parser.add_argument("--ls-ftol", type=float, default=1e-9)
    parser.add_argument("--ls-max-nfev", type=int, default=None)
    parser.add_argument("--coordinate-initialization", choices=("same_snapshot", "previous_time"), default="same_snapshot")
    ...
    main(
        ...
        gamma=args.gamma,
        am_tol=args.am_tol,
        am_max_iter=args.am_max_iter,
        ls_ftol=args.ls_ftol,
        ls_max_nfev=args.ls_max_nfev,
        coordinate_initialization=args.coordinate_initialization,
